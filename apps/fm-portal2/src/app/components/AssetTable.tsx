...
export default function AssetTable({ assets }:{ assets:Asset[] }){
  ...
  const filtered=useMemo(()=>assets.filter(a=>[a.site,a.building,a.location,a.id]
    .some(s=>s.toLowerCase().includes(q.toLowerCase()))),[assets,q]);
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5">
      <div className="p-3">
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search site, building, location…"
